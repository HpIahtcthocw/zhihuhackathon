...
8. 去 AI 腔（违反任何一条即重写）：禁止三连排比；禁止「不是A，而是B」「与其说…不如说…」「真正的…是…」句式；禁止「记住：」式说教；禁止格局/维度/闭环/赋能/底层逻辑这类空词；每一句都要像具体的人在说话——可以有口语、犹豫、不满，甚至说得不完美；
9. 共鸣铁律：三轮质询中至少两轮引用玩家处境的原话（如「你说存款只够撑半年——」）；每个质询至少含困境原文的一个具体名词；desc 禁止「揭示/展现/探讨」导览腔；
9. 只输出 JSON，不要任何解释文字。`;
...
要求：第1个月R选项如果埋了延迟后果，加 "delay":"第N个月：……"（30-50字）；4个月事件必须覆盖不同生活侧面（钱/人际/机会/意外）；endings 的四种命运要有真区别，判词要有余味不说教；crash 和 fence 两种结局允许克制的自嘲式黑色幽默（像人生重开模拟器那样让人笑一下再叹口气），但不得嘲弄真实痛苦。
